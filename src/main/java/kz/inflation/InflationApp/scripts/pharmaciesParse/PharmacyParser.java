package kz.inflation.InflationApp.scripts.pharmaciesParse;
...
@Component @Slf4j
public class PharmacyParser extends Thread {
    ...
    @Autowired
    public PharmacyParser(PharmacyService pharmacyService, PharmacyCategoryService pharmacyCategoryService) {
        this.pharmacyService = pharmacyService;
        this.pharmacyCategoryService = pharmacyCategoryService;
    }

    public PharmacyParser(PharmacyService pharmacyService, PharmacyCategoryService pharmacyCategoryService, String link, String threadName) {
        this.pharmacyService = pharmacyService;
        this.pharmacyCategoryService = pharmacyCategoryService;
    ...
    public PharmacyParser(PharmacyService pharmacyService, PharmacyCategoryService pharmacyCategoryService, List<String> linksList) {
        this.pharmacyService = pharmacyService;
        this.pharmacyCategoryService = pharmacyCategoryService;
    ...
    private void mobileVersion(WebDriver driver, boolean switcher){
        ...
        HashSet<Pharmacy> uniqueProducts = new HashSet<>(this.update(driver.getPageSource()));
        ...
        pharmacyService.saveAll(uniqueProducts.stream().toList());
    }

    private List<Pharmacy> update(String seleniumDocument){
        Document document = Jsoup.parse(seleniumDocument);
        List<Pharmacy> productList = new ArrayList<>();
        if (category == null)
            category = pharmacyCategoryService.getCategory(document.getElementsByClass("catalog-grid-header__heading").text());
...
//</div>
            ...
            Pharmacy product1 = new Pharmacy(articul, name, realPrice);
            if (category == null || category.getId() == null) category = pharmacyCategoryService.getCategory(document.getElementsByClass("catalog-grid-header__heading").text());
